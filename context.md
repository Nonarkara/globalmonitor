...
- **Production URL**: https://global.nonarkara.org/ — Cloudflare Pages project `globalmonitor-me` (globalmonitor-me.pages.dev): static frontend + API via Pages Functions, same origin. **This branch (`middleeast`) is the World Console.** The flagship at globalmonitor.nonarkara.org is branch `classic` → project `globalmonitor`; the Asia build at asia.nonarkara.org is branch `main` → project `asiawatch`. Full table under "Deployment map" at the end of this file.
- **Custom domain**: global.nonarkara.org is attached to project `globalmonitor-me` and live.
...
- **Production**: https://global.nonarkara.org/ (Cloudflare Pages project `globalmonitor-me` — frontend + API; production branch is `middleeast`)
...
- **Committed on URL: https://global.nonarkara.org/** — change is live at that URL (repeat the full URL every time).
...
## Ship tracking (AIS)
- **Pages production fallback (2026-06-23)**: When aisstream WebSocket is silent (`rawSeen=0`, see aisstream/issues#178) and no static snapshot is deployed, `/api/vessels` falls back to **Axiom Overwatch REST** (`axiomoverwatch.io/api/v1/positions/latest`) — no API key, CC-BY 4.0, ~20K global vessels, 15 min cache.
- **Axiom Overwatch**: Free REST global AIS snapshot. Signup: **none required**. Docs: https://docs.axiomancer.io/overwatch/api/vessels — rate limit 60/min, 1,000/day per IP.
...
|---|---|---|---|
| `middleeast` (`v3-middleeast`) — **this one** | `globalmonitor-me` | global.nonarkara.org — the World Console | `npm run deploy:pages` (`--branch=middleeast`); GitHub Actions on push to middleeast |
...
| MEM (`v2-standalone`, separate repo mem-by-non) | `mem-by-non` | mem.nonarkara.org (API base is global.nonarkara.org/api) | direct upload |
